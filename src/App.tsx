...
const CreateCourse = lazy(() => import('./pages/CreateCourse').then(module => ({ default: module.CreateCourse })));
...
const CertificateEditor = lazy(() => import('./pages/CertificateEditor').then(module => ({ default: module.CertificateEditor })));
...
const MainLayout: React.FC = () => {
  ...
  const isSuperAdminRoute = location.pathname.startsWith('/super-admin') || location.pathname.startsWith('/admin-dashboard');

  return (
    <div className="flex flex-col min-h-screen w-full overflow-x-hidden">
      {!isSuperAdminRoute && <Navbar />}
      {!isSuperAdminRoute && <TourOverlay />}
      {isSuperAdminRoute && <AdminChatWidget />}
      {!isSuperAdminRoute && <UserChatWidget />}
      <main className={`flex-grow flex flex-col w-full ${isSuperAdminRoute ? 'h-screen' : ''}`}>
        <Suspense fallback={
          ...
          <Routes>
            ...
            <Route path="/admin-dashboard" element={<AuthGuard><SuperAdminDashboard /></AuthGuard>} />
                  ...
                  <button
                    ...
                    onClick={() => {
                      import('react-toastify').then(({ toast }) => {
                        toast.info("Nos réseaux sociaux arrivent bientôt !");
                      });
                    }}
                    className="w-10 h-10 flex items-center justify-center rounded-full bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 text-gray-500 hover:bg-primary-500 hover:text-white hover:border-primary-500 dark:hover:bg-primary-500 transition-all duration-300"
                  >
